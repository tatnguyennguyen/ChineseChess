...
var chess = (function() {
    ...
    function compareFunction(ob1, ob2) {
    ...
    function alphaBetaSearch(a, b, dep, isQS, isRoot, isCapture, _turn) {
        ...
        captureMove.sort(compareFunction);
        otherMove.sort(compareFunction);
        if (general === -1) {
            return _turn === 0 ? -(BEST_VALUE + depth * 500) : (BEST_VALUE + depth * 500);
        }
        if (opGeneral === -1) {
            return _turn === 0 ? (BEST_VALUE + depth * 500) : -(BEST_VALUE + depth * 500);
        }
        ...
        if (isQS) {
            if (dep === 0) {
                ...
                if (_turn === 0) { //engine
                    var length, i, bestValue = -Infinity,
                        val;
